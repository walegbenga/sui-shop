...
export default function AuthButton() {
  ...
  const ConnectedMenu = ({ label }: { label: string }) => (
            ...
            <Menu.Item>
              {({ active }) => (
                <button
                  ...
                  className={`${active ? 'bg-gray-100 text-gray-900' : 'text-gray-700'} block w-full px-4 py-2 text-left text-sm`}
                >
                  {isEnokiAccount ? 'Logout' : 'Disconnect'}
                </button>
              )}
            </Menu.Item>
          </div>
  ...
  if (account) {
    ...
    return <ConnectedMenu label={label} />;
  }
      ...
      >
        <Menu.Items className="absolute right-0 z-10 mt-2 w-64 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="py-1">
              ...
              <Menu.Item>
                {({ active }) => (
                  <button
                    ...
                    disabled={isConnecting}
                    className={`${active ? 'bg-gray-100 text-gray-900' : 'text-gray-700'} group flex w-full items-center px-4 py-2 text-sm disabled:opacity-50 disabled:cursor-not-allowed`}
                  >
            ...
            {/* Regular wallets — listed directly, no ConnectModal */}
            ...
            {regularWallets.length === 0 && !googleWallet && (
              <div className="px-4 py-3 text-sm text-gray-500 text-center">
                No wallets found. Install a Sui wallet extension.
              </div>
            )}
