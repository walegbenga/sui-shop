...
console.log('🔥 INDEXER SCRIPT STARTED');
...
async function handleSellerFollowed(event: any) {
  try {
    const { seller, follower } = event.parsedJson;
...
// Start the indexer (now returns a never-resolving promise)
startIndexer().catch((error) => {
  console.error('❌ Fatal error:', error);
  process.exit(1);
});

// Start the indexer (REPLACE the existing bottom section)
console.log('🎬 About to start indexer...');

startIndexer()
  .then(() => {
    console.log('✅ Indexer promise resolved');
  })
  .catch((error) => {
    console.error('❌ Fatal error:', error);
